...
class Workflows:
    ...
    def get(self, workflow_id: str, cache: bool = True) -> Workflow:
        cache_key = self._cache_key(CacheRetrieveType.GET, id=workflow_id)
        if cache:
...
class WorkflowSteps:
    ...
    def get(self, workflow_id: str, step_id: str, cache: bool = True) -> WorkflowStep:
        cache_key = self._cache_key(CacheRetrieveType.GET, id=(workflow_id, step_id))
        if cache:
            cache_data = self._cache.get(cache_key)
            if cache_data is not None:
